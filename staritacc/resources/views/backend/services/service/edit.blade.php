                ...
                <div class="tile-body tile-footer">
                    ...
                    <form method="post" action="{{ route('service.update',$services->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" type="text" placeholder="Name" name="name" value="{{$services->name}}">
                            ...
                            <div class="col-md-8">
                                <select name="service_unit_id" id="service_unit_id" class="form-control">
                                    ...
                                    @foreach($units as $unit)
                                        <option value="{{$unit->id}}"  {{$unit->id == $services->service_unit_id ? 'selected' : ''}}>{{$unit->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-8">
                                <select name="service_sub_category_id" id="service_sub_category_id" class="form-control">
                                    ...
                                    @foreach($subCategories as $subCategory)
                                        <option value="{{$subCategory->id}}" {{$subCategory->id == $services->service_sub_category_id ? 'selected' : ''}}>{{$subCategory->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <img src="{{asset('uploads/service/'.$services->image)}}" alt="{{$services->name}}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <textarea rows="4" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" type="text" placeholder="description" name="description"> {!! $services->description  !!}</textarea>
                            </div>
                        </div>
                        <div class="form-group row">
